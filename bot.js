...
client.on('message', msg => {  //WESO#0001
    if (msg.content === 'باك') {  //WESO#0001
      msg.reply('** :wink: ولكم   :sparkling_heart:**');  //WESO#0001
    }
  });
...
client.on('ready', function(){
    var ms = 10000 ;
    var setGame = [' -help',`Developer By PvPexMc`,`PvPex System Bot`,' PvPex Best Server 💗 ','Hello do You Want Help ? : -help'];
    var i = -1;
    var j = 0;
...
client.on('message', message => {
    if (message.content.startsWith(prefix + 'sug')) {
        ...
        if (!args) return message.reply('اكتب اقتراحك لو سمحت ').then(c => {c.delete(5000)})
